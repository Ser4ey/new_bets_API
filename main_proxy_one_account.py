...
class GetNewAccountsListHigh:
    ...
    def get_new_accounts_list(self):
        '''Возвращает один bet365 аккаунт'''
        list_of_start_info = []
        ...
        with open(data.path_to_accounts_file, 'r', encoding='utf-8') as file:
            ...
            for i in range(len(accounts_)):
                if len(accounts_[i].strip()) != 0:
                    ...
                    bet365_open_site = test_proxy.open_one_proxy_account()

                    driver_class_ok = FireFoxDriverMainNoAutoOpen(bet365_open_site,
                                                                  bet_value=data_[2],
                                                                  login=data_[0],
                                                                  password=data_[1]
                                                                  )

                    log_in_driver(driver_class_ok)
                    return [driver_class_ok]
